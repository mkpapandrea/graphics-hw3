...
void copy_flocker_states_to_graphics_objects()
{
  ...
  xyz_velocities.resize(num_flockers);
  ...
  for (int i = 0; i < num_flockers; i++) {
    ...
    xyz_velocities[i] = glm::vec3(flocker_array[i]->velocity.x, flocker_array[i]->velocity.y, flocker_array[i]->velocity.z);
  }
}
...
void bullet_add_obstacles()
{
  ...
  btCollisionShape* leftShape = new btStaticPlaneShape(btVector3(1, 0, 0), 0);
  ...
  btCollisionShape* rightShape = new btStaticPlaneShape(btVector3(-1, 0, 0), 0);

  btDefaultMotionState* rightMotionState = new btDefaultMotionState(btTransform(btQuaternion(0, 0, 0, 1), btVector3(box_width, 0, 0)));
...
void bullet_add_dynamic_objects()
{
  ...
  btVector3 simplifiedShapeInertia(0, 0, 0);   // if we leave it as all 0's -> treat like point mass -> no spin
  ...
  for (int i = 0; i < num_flockers; i++) {
    ...
    int velocity = 50;
    rigidBody->setLinearVelocity(btVector3(xyz_velocities[i].x * velocity, xyz_velocities[i].y, xyz_velocities[i].z));
